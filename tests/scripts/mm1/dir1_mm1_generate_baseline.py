#!/usr/bin/env python3
"""
Generate baseline data for -dir1 + -mm 1 tests. (V7.0 Compliant)
...
def normalize_path_in_line(line):
    ...
    # Strip directory prefixes before ANY filename.ext: pattern (not just .pdb)
    line = re.sub(r'(?:[\w.\-]+[/\\])+([\w.\-]+\.\w+:)', r'\1', line)
    # Strip leading / or \ before filename.ext:
...
def main():
    print("=" * 60)
    print("Generate Baseline: -dir1 + -mm 1 (Single-Pair Only)")
    print("=" * 60)

    cases = read_regression_cases()
    print(f"\nRegression test cases: {len(cases)} entries")
