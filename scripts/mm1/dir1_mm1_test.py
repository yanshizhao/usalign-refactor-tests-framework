#!/usr/bin/env python3
"""
Test runner for -dir1 + -mm 1 feature. (V7.1 Compliant)
...
REGRESSION_CASES_FILE = os.path.join(SCRIPT_DIR, "dir1_mm1_test_cases.txt")
FEATURE_CASES_FILE = os.path.join(SCRIPT_DIR, "dir1_mm1_feature_cases.txt")
BASELINE_DIR = os.path.join(SCRIPT_DIR, "dir1baseline")
RESULT_DIR = os.path.join(SCRIPT_DIR, "dir1result")
DIFF_DIR = os.path.join(SCRIPT_DIR, "dir1diff")
...
def run_batch_tests(batches, reg_map):
    ...
    for batch in batches:
        ...
        if outfmt == 2:
            ...
            # V7.1 FIX: Use started flag to skip lines before first block marker
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="Test runner for -dir1 + -mm 1 (V7.1)")
    parser.add_argument("--branch", default="USalign-beta", help="Target branch to test")
    args = parser.parse_args()

    print("=" * 60)
    print("Test Runner: -dir1 + -mm 1 (V7.1)")
    print("=" * 60)
    ...
    if not os.path.exists(BASELINE_DIR):
        ...
        print(f"Run generate_dir1_mm1_baseline.py first!"); sys.exit(1)
