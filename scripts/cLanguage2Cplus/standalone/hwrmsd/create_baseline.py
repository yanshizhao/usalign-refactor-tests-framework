#!/usr/bin/env python3
"""
HwRMSD baseline creation script
Switch to the clean master branch, extract unmodified source code, compile the original HwRMSD, run all test cases, and save output to baseline/.
...
def run_baseline(exe_path: str):
    ...
    with open(testcases, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # HwRMSD -o PDB1.sup produces PDB1.sup and PDB1.sup.pml
            sup_file = workdir / "PDB1.sup"
            if sup_file.exists():
                shutil.move(str(sup_file), str(BASELINE_DIR / f"{name}.sup"))

            # HwRMSD -m matrix.txt produces a matrix file
...
if __name__ == "__main__":
    ...
    try:
        if original_branch != "master":
            print(f"Switching USalign from {original_branch} to master for HwRMSD baseline...")
            checkout("master")
